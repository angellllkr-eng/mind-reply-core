...
import { ArrowRight, ChevronRight, Heart, Menu, Play, Search, ShieldCheck, Sparkles, Trophy, User, Wallet, X, Zap } from 'lucide-react'
...
export default function Page() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <header className="site-header"><div className="header-primary"><a className="wordmark" href="#top" aria-label="NOVA home"><span className="wordmark-mark">N</span><span>NOVA<span className="wordmark-dot">.</span></span></a><nav className="desktop-nav" aria-label="Primary navigation"><a className="active" href="#games">Casino</a><a href="#live">Live tables</a><a href="#games">Slots</a><a href="#jackpot">Jackpots</a><a href="#rewards">Promotions</a><a href="#rewards">Rewards</a></nav><label className="header-search"><Search size={16} /><span className="sr-only">Search games</span><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') document.getElementById('games')?.scrollIntoView({ behavior: 'smooth' }) }} placeholder="Search 1,000+ games" /></label><div className="header-actions"><div className="balance-pill"><span className="balance-amount">{currency === 'EUR' ? '€' : currency === 'GBP' ? '£' : '$'}0.00</span><select className="currency-select" value={currency} onChange={(event) => setCurrency(event.target.value)} aria-label="Select currency"><option>EUR</option><option>GBP</option><option>USD</option></select></div><button className="deposit-button" onClick={() => setModal('register')}><Wallet size={15} /> Deposit</button><button className="login-button" onClick={() => setModal('login')}><User size={15} /> Log in</button><button className="menu-button" aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><Menu size={20} /></button></div></div><div className="header-ribbon"><div className="ribbon-tabs">{categories.map((item) => <button key={item} className={category === item ? 'ribbon-tab selected' : 'ribbon-tab'} onClick={() => { setCategory(item); document.getElementById('games')?.scrollIntoView({ behavior: 'smooth' }) }}>{item}</button>)}</div><div className="ribbon-jackpot"><Trophy size={14} /> <span>Jackpot</span> <strong>€184,920.72</strong></div></div></header>
